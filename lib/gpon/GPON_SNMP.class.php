...
class GPON_SNMP
{
	...
	var $options=array();
	var $error=array();
	var $path_OID='SLE-GPON-MIB::';
	function GPON_SNMP($options=array(),&$CONFIG,&$GPON)
	{
		$this->CONFIG = &$CONFIG;
		$this->GPON = &$GPON;
...
//		@snmp_read_mib('sle-systemmaintenance-mib.mib');
	...
	{
		$result='';
		if(ConfigHelper::getConfig('phpui.gpon_tx_output_power_weak'))
		{
			$rxpower=(float)str_replace(',','.',str_replace('dBm','',$rxpower));
			$gpon_tx_output_power_weak=(float)str_replace(',','.',str_replace('dBm','',ConfigHelper::getConfig('phpui.gpon_tx_output_power_weak')));
	...
	function ONU_get_param_table($OLT_id,$ONU_id,$ONU_name='')
	{
		...
		if($this->ONU_is_real($OLT_id,$ONU_id,$ONU_name))
		{
			...
			if(is_array($snmp_result) && count($snmp_result)>0)
			{
				...
				if(!ConfigHelper::getConfig('phpui.view_onu_passwords'))
				{
